...
def create_token_pair(user: models.User) -> schemas.TokenPair:
    payload = {config.SUB: str(user.id), config.JTI: str(uuid.uuid4()), config.IAT: _get_utc_now()}
...
def get_user_from_access_token(authorization: str| None = Header(None), db: Session = Depends(get_db)) -> int:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    print(f"Error with response {url}. Response status: {response.status}")
