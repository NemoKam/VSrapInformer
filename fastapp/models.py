...
collection_product_table = Table(
    ...
    Base.metadata,
    Column("collection_id", ForeignKey("collection_table.id", ondelete="CASCADE")),
...
user_combination_table = Table(
    ...
    Column("combination_id", ForeignKey("combination_table.id", ondelete="CASCADE")),
)
...
class Code(Base):
    ...
    type: Mapped[ENUM] = mapped_column(ENUM("email", "phone_number", name="code_type_enum", create_type=False))
    code: Mapped[str]
...
class User(Base):
    ...
    expire_datetime: Mapped[datetime] = mapped_column(default=datetime.now() + timedelta(minutes=config.UNVERIFIED_USER_EXPIRES_MINUTES), nullable=True)
...
class Collection(Base):
    ...
    title: Mapped[str]
    products: Mapped[list["Product"]] = relationship(secondary=collection_product_table, back_populates="collections", lazy="dynamic")
